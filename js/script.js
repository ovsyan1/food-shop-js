window.addEventListener("DOMContentLoaded", () => {
  ...
  function setClock(selector, endtime) {
    ...
    updateClock(); //чтобы не было мигания после перезагрузки
  ...
  class MenuCard {
    constructor(src, alt, title, descr, price, parentSelector, ...classes) {
      ...
      this.parent = document.querySelector(parentSelector); //дома элемент
      this.transfer = 27;
      this.changeToUAH();
    }
  ...
  function addZeroToCurrent(elements){
    if(elements < 10){
  ...
  // addZeroToCurrent(slides.length);
  // }
  ...
  function calcTotal(){
    if(!sex || !height || !weight || !age || !ratio){
      result.textContent = `Заполните все поля, для получения`;
      return; // прерываем функцию
    }
    if(sex === 'female'){
      result.textContent = Math.round((447.6 + (9.2 * weight) + (3.1 * height) - (4.3 * height) - (4.3 * age)) * ratio);
    }else{
